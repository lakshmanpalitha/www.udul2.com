...
class Megnor_AdvancedMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawAdvancedMenuItem($category, $level = 0, $last = false)
    {
        ...
        if ($drawPopup)
        {
            $html[] = '<div id="menu' . $id . '" class="menu icon'.$i++.' menu-arrow' . $active . '" onmouseover="megnorShowMenuPopup(this, \'popup' . $id . '\');" onmouseout="megnorHideMenuPopup(this, event, \'popup' . $id . '\', \'menu' . $id . '\')">';
        }
        else
        {
            $html[] = '<div id="menu' . $id . '" class="menu' . $active . ' icon'.$i++.'">';
        }
        // --- Top Menu Item ---
        ...
        $html[] = '<span data-hover="'.$name.'">' . $name . '</span>';
        $html[] = '</a>';
        $html[] = '</div>';
        $html[] = '</div>';
